...
const calcVEha     = (bicos,vel,esp) => (!bicos||!vel||!esp)?0:(bicos*VAZAO_BICO)/((vel*1000/60)*esp)*10000;
...
const C = {
  ...
  blue:"#60a5fa",blueBg:"#0a1a2a",  // cor segunda aplicação simultânea
};
...
const btnP= {background:C.gr,color:C.bg,border:"none",borderRadius:12,padding:"12px 16px",fontSize:14,fontWeight:800,width:"100%",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:7};
...
const Bdg = ({v}) => {
  ...
  const a=Math.abs(v); const [bg,fg]=a<=5?[C.okBg,C.ok]:a<=15?[C.warnBg,C.warn]:[C.errBg,C.err];
...
export default function App() {
  ...
  const [novaOpFaz, setNovaOpFaz] = useState("");
  const [novaEq,    setNovaEq]    = useState("");
  const [novaEqFaz, setNovaEqFaz] = useState("");
  ...
  const criarAp = async () => {
    ...
    const nova={id,seq:nextSeq,fazenda:nfaz,talhoes:ntals.map(t=>t.cod),status:"aberta",dataCriacao:today(),apontamentos:[]};
    ...
    setTela("apontamento");
    try {
      await sbPost("aplicacoes",{id,seq:nova.seq,id_subfazenda:nfaz,status:"aberta",data_criacao:today()});
  ...
  const salvarApt = async () => {
    ...
    try {
      ...
      await sbPost("apontamentos",{id:apId,id_aplicacao:ap.id,data:aData,id_operador:opObj?.id||null,id_equipamento:eqObj?.id||null,vol_total:volTotal,vel_media:velM,bicos_media:bicosM,observacao:aObs,cancelado:false});
  ...
  if(tela==="entrada") return (
    <div style={{...AppStyle,background:"#040a04",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 24px",position:"relative"}}>
      ...
      <div style={{position:"relative",zIndex:1,width:"100%",display:"flex",flexDirection:"column",alignItems:"center"}}>
        <div style={{width:72,height:72,background:"#0f1f0f",border:`1.5px solid ${C.grDim}`,borderRadius:20,display:"flex",alignItems:"center",justifyContent:"center",marginBottom:20}}><ILeaf/></div>
        ...
        <div style={{width:"100%"}}>
          ...
          {fazendas.map(faz=>(
            <button key={faz.id} onClick={()=>{setFazSel(faz);setSubf(faz.subfazendas[0]?.id||"");setNfaz(faz.subfazendas[0]?.id||"");carregar(faz);setAba("talhoes");setTela("main");}}
        ...
        <div style={{fontSize:10,color:C.txM,marginTop:20,letterSpacing:1}}>v8.0 · 8 bar · 1,6 L/min por bico</div>
      </div>
  ...
  if(tela==="main") {
    ...
    const TabAplicacoes=()=>{
      const subfIds=fazSel?.subfazendas.map(s=>s.id)||[];
      const lista=aplicacoes.filter(ap=>subfIds.includes(ap.fazenda)).sort((a,b)=>b.seq-a.seq);
      return (
        <div style={{padding:"12px 12px 88px"}}>
    ...
    const TabPainel=()=>{
      const subfIds=fazSel?.subfazendas.map(s=>s.id)||[];
      const todas=aplicacoes.filter(ap=>subfIds.includes(ap.fazenda));
            ...
            {tals.map(t=>{
              ...
              return (
                <div key={t.cod} style={{padding:"8px 0",borderBottom:`1px solid ${C.bor2}`}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                    <div><span style={{fontSize:13,fontWeight:800}}>Talhão {t.quadra}</span><span style={{fontSize:10,color:C.txM,marginLeft:5}}>{parseFloat(t.area).toFixed(2)} ha · rua {t.esp_rua}m</span></div>
                    <div style={{textAlign:"right"}}><span style={{fontSize:13,fontWeight:700,color:C.gr}}>{fmtL(real)}</span><span style={{fontSize:10,color:C.txD}}> / {fmtL(esp)}</span>{devT!==null&&<div><Bdg v={devT}/></div>}</div>
                  </div>
                  <Bar pct={pctT} h={4}/>
                </div>
              );
                ...
                <div style={{display:"flex",justifyContent:"space-between"}}>
                  <div>
                    <div style={{display:"flex",alignItems:"center",gap:6}}>
                      <span style={{fontSize:12,fontWeight:700,color:C.txD}}>#{i+1} · {r.data}</span>
                    ...
                    <div style={{fontSize:10,color:C.txM}}>{calcVelPond(r.trechos).toFixed(2)} km/h · {calcBicosPond(r.trechos).toFixed(0)} bicos</div>
  ...
  if(tela==="config") {
    const menuItens=[
      ...
      {id:"talhoes_cfg", label:"Talhões",               sub:`${talhoes.length} cadastrados`},
    ...
    return (
      <div style={AppStyle}>
        ...
        <div style={{padding:12}}>
            ...
            <div>
              ...
              <div style={crd}>
                ...
                <select value={ntSubf} onChange={e=>setNtSubf(e.target.value)} style={{...sel,marginBottom:8}}>
                  <option value="">Selecionar...</option>
                  {fazendas.flatMap(f=>f.subfazendas).map(s=><option key={s.id} value={s.id}>{s.sigla} — {s.nome}</option>)}
                </select>
                ...
                <button onClick={async()=>{
                  ...
                  try{
                    ...
                    setTalhoes(p=>[...p,{cod:ntCod.trim(),id_subfazenda:ntSubf,quadra:ntQ.trim(),variedade:ntVar.trim(),plantas:parseInt(ntPl),esp_rua:fv(ntRua),esp_planta:fv(ntPlE),area}]);
          ...
          {secao==="operadores"&&(
            <div>
              <div style={crd}>
                ...
                {operadores.map(op=>(
                  <div key={op.id} style={{padding:"8px 0",borderBottom:`1px solid ${C.bor2}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div><div style={{fontSize:13,fontWeight:600}}>{op.nome}</div></div>
                    <button onClick={async()=>{try{await sbPatch("operadores",`id=eq.${op.id}`,{ativo:false});setOperadores(p=>p.filter(x=>x.id!==op.id));}catch(e){alert("Erro: "+e.message);}}} style={{background:"none",border:"none",color:C.err,cursor:"pointer",fontSize:11}}>Remover</button>
                  </div>
                ))}
              </div>
              <div style={crd}>
                <span style={lbl}>Novo operador</span>
                ...
                <select value={novaOpFaz} onChange={e=>setNovaOpFaz(e.target.value)} style={{...sel,marginBottom:10}}>
                  <option value="">Selecionar...</option>
                  {fazendas.map(f=><option key={f.id} value={f.id}>{f.nome}</option>)}
                </select>
                <button onClick={async()=>{
                  if(!novaOp.trim()||!novaOpFaz) return;
                  try{
                    const res=await sbPost("operadores",{nome:novaOp.trim(),ativo:true});
                    await sbPost("operador_fazenda",{id_operador:res[0].id,id_fazenda:novaOpFaz});
                    setOperadores(p=>[...p,res[0]]); setNovaOp(""); setNovaOpFaz("");
                  }catch(e){alert("Erro: "+e.message);}
              ...
              <div style={crd}>
                ...
                {equipamentos.map(eq=>(
                  <div key={eq.id} style={{padding:"8px 0",borderBottom:`1px solid ${C.bor2}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    <button onClick={async()=>{try{await sbPatch("equipamentos",`id=eq.${eq.id}`,{ativo:false});setEquipamentos(p=>p.filter(x=>x.id!==eq.id));}catch(e){alert("Erro: "+e.message);}}} style={{background:"none",border:"none",color:C.err,cursor:"pointer",fontSize:11}}>Remover</button>
                  </div>
                ))}
              </div>
              <div style={crd}>
                <span style={lbl}>Novo equipamento</span>
                <input placeholder="Nome" value={novaEq} onChange={e=>setNovaEq(e.target.value)} style={{...inp,marginBottom:8}}/>
                <span style={lbl}>Fazenda</span>
                <select value={novaEqFaz} onChange={e=>setNovaEqFaz(e.target.value)} style={{...sel,marginBottom:10}}>
                  <option value="">Selecionar...</option>
                  {fazendas.map(f=><option key={f.id} value={f.id}>{f.nome}</option>)}
                </select>
                <button onClick={async()=>{
                  if(!novaEq.trim()||!novaEqFaz) return;
                  try{
                    const res=await sbPost("equipamentos",{nome:novaEq.trim(),ativo:true});
                    await sbPost("equipamento_fazenda",{id_equipamento:res[0].id,id_fazenda:novaEqFaz});
                    setEquipamentos(p=>[...p,res[0]]); setNovaEq(""); setNovaEqFaz("");
                  }catch(e){alert("Erro: "+e.message);}
          ...
          {secao==="senha"&&(
            <div style={crd}>
              <span style={lbl}>Nova senha (4 dígitos)</span>
              ...
              }} style={{...btnP,opacity:novaSenha.length===4?1:0.4}}>Salvar nova senha</button>
            </div>
          )}
        </div>
      </div>
