...
export default function App() {
  ...
  if(tela==="config") {
    const menuItens=[
      ...
      {id:"sobre",       label:"Sobre o sistema",        sub:"v8.5 · Banco · Repositório · Contexto"},
    ];
    return (
      <div style={AppStyle}>
          ...
          {secao==="operadores"&&(
            <div>
                        ...
                        <div style={{display:"flex",gap:8}}>
                          <button onClick={async()=>{
                            ...
                            try{
                              ...
                              setOperadores(p=>p.map(x=>x.id!==op.id?x:{...x,nome:editOpNome.trim(),matricula:editOpMat.trim()||null}));
                              setEditOp(null);
                            }catch(e){alert("Erro: "+e.message);}
              ...
              <div style={crd}>
                ...
                <input placeholder="Matrícula (opcional)" value={novaOpMat} onChange={e=>setNovaOpMat(e.target.value)} style={{...inp,marginBottom:8}}/>
                ...
                <button onClick={async()=>{
                  ...
                  try{
                    ...
                    setOperadores(p=>[...p,res[0]]); setNovaOp(""); setNovaOpFaz(""); setNovaOpMat("");
                  }catch(e){alert("Erro: "+e.message);}
                ...
                {fazendas.map(faz=>{
                  ...
                  return (
                    <div key={faz.id} style={{padding:"10px 0",borderBottom:`1px solid ${C.bor2}`,display:"flex",alignItems:"center",gap:10}}>
                      ...
                      }} style={{background:C.gr,color:C.bg,border:"none",borderRadius:9,padding:"8px 14px",fontSize:12,fontWeight:700,cursor:"pointer",whiteSpace:"nowrap"}}>
                        Salvar
                      </button>
                    </div>
                  );
